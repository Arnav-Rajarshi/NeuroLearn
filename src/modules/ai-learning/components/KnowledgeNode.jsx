...
function ContentSection({ icon: Icon, title, content, color }) {
  ...
  return (
    <div className="mb-6">
      ...
      <p className="text-[var(--color-muted)] leading-relaxed pl-10">
        {content}
      </p>
    </div>
  )
        ...
        <div className="mt-8 p-4 rounded-xl bg-[var(--color-surface-raised)] border border-[var(--color-border)]">
          ...
          <div className="flex flex-wrap gap-2">
            {node.subtopics.slice(0, 5).map((subtopic, i) => (
              <span
                key={i}
                className="px-3 py-1 text-xs rounded-full bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-muted)]"
              >
                {subtopic}
              </span>
            ))}
