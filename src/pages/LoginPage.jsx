...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (emailInputRef.current) {
        emailInputRef.current.style.animation = "shake 0.5s"
      }
      if (passwordInputRef.current) {
        passwordInputRef.current.style.animation = "shake 0.5s"
      }
      setTimeout(() => {
        if (emailInputRef.current) emailInputRef.current.style.animation = ""
        if (passwordInputRef.current) passwordInputRef.current.style.animation = ""
      }, 500)
    } finally {
      setLoading(false)
    }
  ...
  // Ambient particle positions (deterministic)
  const particles = [
